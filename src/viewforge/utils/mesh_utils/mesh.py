    ...
    **kwargs,
    ...
    def p_func(tree, task_id_in=task_id):
        if "nodes" in tree and tree["nodes"][0]["name"].find("world") >= 0:
            tree["nodes"].pop(0)
        tree["asset"]["generator"] = "asset_smith"
        change_dict = {
            "nodes": "node",
            "meshes": "mesh",
            "materials": "mat",
        ...
        for k, v in change_dict.items():
            if k in tree:
                for index in range(len(tree[k])):
                    tree[k][index]["name"] = f"atlas_{v}_{task_id_in}"
...
def replace_mesh_texture_and_save_gltflib(
    ...
    **kwargs,
    ...
    gltf.model.nodes[0].name = f"atlas_node_{task_id}"
    gltf.model.meshes[0].name = f"atlas_mesh_{task_id}"
    gltf.model.materials[0].name = f"atlas_material_{task_id}"
    gltf.model.asset.generator = "asset_smith"
